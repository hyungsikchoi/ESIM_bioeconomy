...
$IFTHEN "%Lignomodule%"=="yes"

   diff_lino = ord(sim_base);
...
*   pop_gr(cc)  = popul_grw(cc,sim_base);
   pop_gr(cc)  = pop_gr(cc) * popul_grw(cc,sim_base);
   ...
   eng_ind(cc) = (1.0 + indices("energy",cc)/100)**(ord(sim_base)-1);
   frt_ind(cc) = (1.0 + indices("fertilizer",cc)/100)**(ord(sim_base)-1);
...
****andre change 2050
   pdem_tr(cc,'rapseed')   = pdem_tr(cc,'rapseed') *(1+ pdem_shft_os(sim_base,cc)/100);
   pdem_tr(cc,'sunseed')   = pdem_tr(cc,'sunseed') *(1+ pdem_shft_os(sim_base,cc)/100);
If (ord(sim_base) gt 1,
*   pdem_tr(cc,'soybean')   = pdem_tr(cc,'soybean') *(0.99);
**hyungsik changed to 1
   pdem_tr(cc,'soybean')   = pdem_tr(cc,'soybean') *(1);
   ...
   LOOP(switch_comm,
      IF(ORD(sim_base) gt 1,
         IF(pol_set_thresh(switch_comm,sim_base) - pol_set_thresh(switch_comm,sim_base-1) ne 0.0,
            ...
            sp_d(one,switch_comm)$member(one)     = bound_tar_cc(one,switch_comm,'applied') * SUM(euro1, exrate(EURO1))/exrate(one);
...
* Pre-determined changes of intervention and threshold prices
   LOOP(trq_comm,
      IF(chg_TRQ_pre(trq_comm,sim_base) ne 0,
          TRQ(trq_comm)  = TRQ0(trq_comm) * (1+chg_TRQ_pre(trq_comm,sim_base) / 100);
          TRQ0(trq_comm) = TRQ(trq_comm);
      );
   );
...
*achtung
*      quota("IE","SETASIDE")$volsaq("IE","SETASIDE") = quota("IE","SETASIDE")/10 ;
*      quota("UK","SETASIDE")$volsaq("IE","SETASIDE") = quota("UK","SETASIDE")/10 ;
*      quota("NL","SETASIDE")$volsaq("IE","SETASIDE") = quota("NL","SETASIDE")/10 ;
*      quota("PT","SETASIDE")$volsaq("IE","SETASIDE") = quota("PT","SETASIDE")/10 ;
   );
* Trigger to abolish quota regime (Default value 1.0)
   ...
   Current_year=(ord(sim_base))+2007;
   display current_year;
   display chg_quota_sim2;
*==== DIRECT PAYMENTS ====*

display chg_dp,defl_pol,Art_change;
...
* Art. 68 payments
      ...
      display ENV_ART_69,Art_change,artificial_value;

      ENV_ART_69(EU27,comm)$(ENV_ART_69(EU27,comm) lt 0) = 0;

      Loop(eu27,
         loop(comm,
            ABORT$(ENV_ART_69(EU27,comm) lt -0.0001) "ART 69 payments can't be negative",env_art_69;
         );
      );
...
*      If ((chk_sim - year_shift(sim_base)) eq 0,

*           exog_land2(EU27)      = Exog_Woodycrops(EU27,sim_base);
*           exog_biogas(eu27,biogas_inputs) = exog_biogas_dem(eu27,biogas_inputs,sim_base);
*          biofuels_additional are expressed in relation to hdem.l
*           biofuels_ext(EU27,energ)         = biofuel_additional(sim_base,energ);
*           );
...
*************************************************
   ...
   simpar('diff_lino','EU_to','LINO',sim_base) = diff_lino;
*   simpar('exog_biof',EU27,'OTHER',sim_base)        = Exog_land2(EU27);
*   simpar('biofuels_ext',EU27,energ,sim_base) = biofuels_ext(EU27,energ);
*   simpar('exog_biogas',eu27,biogas_inputs,sim_base) = exog_biogas(eu27,biogas_inputs);
* end loop sim_base
);
