...
*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
*! DO NOT MODIFY ANYTHING IN THE FOLLOWING CODE !
*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
* switch between calibration procedure and ESIM simulations (CALSIM=no: normal ESIM simulation):
$IFTHEN "%calibrateToAglink%"=="yes"
*  during calibration, also switch calibrated shifters on:
$  SETGLOBAL useCalibratedShifters yes
   PARAMETER CALSIM /yes/;
$ELSE
   PARAMETER CALSIM /no/;
$ENDIF
...
$IFTHEN "%useCalibratedShifters%"=="yes"
   PARAMETER USECALPAR /yes/;
$ELSE
...
* interpolating the years between base and INTERPOL_UPTO (only for calibration or calibrated simulation):
$SETGLOBAL INTERPOL_UPTO "'2011'";
$IFTHEN "%useCalibratedShifters%"=="yes"
   PARAMETER INTERPOL2020 /yes/;
$ELSE
...
$SETGLOBAL CAL_CONTROL_FILE "../cal_control_bioeconomy.xls";
...
SETS
  sim_run(sim_base)        /base, 2009*2050/
...
PARAMETER
  ...
  simres(SIMRESSET, ccplus, i, sim_base)      simulation results per country in level terms
  simrespch(SIMRESSET, ccplus, i, sim_base)   simulation results as percentage change from base
  simresagg(SIMRESSET, EUGRP, i, sim_base)    simulation results per country group in level terms
  ...
  atQuota(cc,comm,sim_base)                   1 for commodities at quota limit
  quota_toggle(cc,comm,sim_base)              1 if quota is active
...
$IFTHEN "%calibrateToAglink%"=="yes"
*     only read new cal_control file for a new calibration
*     keep the previous cal_control settings if calibrated parameters are used to simulate
...
display tPW_CS,SUPPLY_CS,PW_CS,it,TUSE_CS,SUPPLY_CS;
...
*display FR, PD.L, PP.L, FRATE.L, fedres, SUPPLY.L;
...
display TP_GR_NONEU.l;

SET QUOTA_YEARS(sim_base) years when quota applied /base, 2009*2050/;
...
**check p0
   ...
   diff_lino = simpar('diff_lino','EU_to','LINO',sim_run);
*   exog_biof(one) = simpar('exog_biof',one,'OTHER',sim_run);
*$offtext
*   biofuels_ext(EU27,energ) = simpar('biofuels_ext',EU27,energ,sim_run);
*   exog_biogas(eu27,biogas_inputs) = simpar('exog_biogas',eu27,biogas_inputs,sim_run);

   cost_ind('INTERMED',cc) = int_ind(cc);
   cost_ind('LABOR',cc) = lab_ind(cc);
   cost_ind('CAPITAL',cc) = cap_ind(cc);
   cost_ind('ENERGY',cc) = eng_ind(cc);
   cost_ind('FERTIL',cc) = frt_ind(cc);
...
***ligno module
   ...
   IF(ORD(sim_run) eq 2.0,
     calib=0;
     Peak_LINO_Area(one)=1;
    );

   IF(ORD(sim_run) gt 2.0,
     calib=0;
     Peak_LINO_Area(one)=Peak_LINO_Area_scen(one);
...
*           apply calibrated efficiency parameter set:
            if(sameas(sim_run,'base'),
               ...
               loop(INTERPOLS$(INTERPOLS.val=interpol),
                  ...
                  simpar('tp_gr',cc,comm,sim_run)$(NOT simpar('tp_gr',cc,comm,sim_run)) = 1;
                  simpar('hdem_tr',cc,comm,sim_run)$(NOT simpar('hdem_tr',cc,comm,sim_run)) = 1;
                  simpar('pdem_tr',cc,comm,sim_run)$(NOT simpar('pdem_tr',cc,comm,sim_run)) = 1;
                  simpar('fdem_tr',cc,comm,sim_run)$(NOT simpar('fdem_tr',cc,comm,sim_run)) = 1;
*                 combine efficiency parameters from calibrated parameters and macro input data as included in simpar:
                  tp_gr(cc,comm)   = simpar('tp_gr',cc,comm,sim_run)   * calparinterpol('tp_gr_cal', cc, comm, sim_run, INTERPOLS);
                  hdem_tr(cc,comm) = simpar('hdem_tr',cc,comm,sim_run) * calparinterpol('hdem_tr_cal', cc, comm, sim_run, INTERPOLS);
                  pdem_tr(cc,comm) = simpar('pdem_tr',cc,comm,sim_run) * calparinterpol('pdem_tr_cal', cc, comm, sim_run, INTERPOLS);
                  fdem_tr(cc,comm) = simpar('fdem_tr',cc,comm,sim_run) * calparinterpol('fdem_tr_cal', cc, comm, sim_run, INTERPOLS);
...
*hdem.fx(eu27,"ethanol")     =  BF_fix(eu27,"ethanol")   + (biofuels_ext(EU27,"ethanol")   * BF_fix(eu27,"ethanol")  );
*hdem.fx(eu27,"biodiesel")   =  BF_fix(eu27,"biodiesel") + (biofuels_ext(EU27,"biodiesel") * BF_fix(eu27,"biodiesel"));
...
simresagg('PD_euro', EU15EU12, i, sim_run)$sum(EUGRPMEM(EU15EU12,cc), simres('TUSE', cc, i, sim_run))
                               = sum(EUGRPMEM(EU15EU12,cc), simres('PD_euro', cc, i, sim_run)*simres('TUSE', cc, i, sim_run))
                                                 / sum(EUGRPMEM(EU15EU12,cc), simres('TUSE', cc, i, sim_run));
...
if(CALSIM OR USECALPAR,
   ...
   execute_unload 'cal_diagnostic_output.gdx' rest_quota, rest_quota_agg, atQuota, atLowerBound, shifterAlert, devtgt;
);

if((CALSIM) AND NOT sum((SIMRESSET,CTRYGRP,comm,sim_run)$(NOT sameas(sim_run,'base')), devtgt(SIMRESSET,CTRYGRP,comm,sim_run)) eq 0, abort "Not all targets, see parameter devtgt!",devtgt);
*++++++++++++++++
*++++++++++++++++
